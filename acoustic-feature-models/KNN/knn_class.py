...
class KNNModel:
    ...
    def evaluate_k(self, k, do_normalisation, do_fs, fs_type, n_folds):
        X_df = self.X
        Y_df = self.Y
        # normalisation
        if (do_normalisation): X_df = self.normalise(X_df)
        Y_df.to_numpy()
        ...
        for i, [train_index, test_index] in enumerate(kf.split(X_df)):
            ...
            if (do_fs) :
                if (fs_type == "uni"):
                    X_train, X_test = self.univariate_selection(X_train, y_train, X_test)
                elif (fs_type == "recursive"):
                    X_train, X_test = self.recursive_selection(X_train, y_train, X_test)
                elif (fs_type == "rf"):
                    X_train, X_test = self.rf_selection(X_train, y_train, X_test)
                elif (fs_type == "overlap"):
                    pass
    ...
    def plot_k_accuracy(self, k_max, do_normalisation, do_fs, fs_type, n_folds):
        accuracies = []
        k_values = range(1, k_max)
        for k in k_values:
            acc_score = self.evaluate_k(k, do_normalisation, do_fs, fs_type, n_folds)
